use crate::egui;
use crate::egui::{Color32, Stroke};

pub const ACCENT: Color32 = Color32::from_rgb(94, 160, 235);      // azzurro, per stati "attivo/selezionato"
pub const ACCENT_DIM: Color32 = Color32::from_rgb(45, 75, 110);   // stroke inattivo, stesso hue smorzato
pub const TEXT_PRIMARY: Color32 = Color32::from_rgb(230, 230, 230);
pub const TEXT_SECONDARY: Color32 = Color32::from_gray(150);
...
// ---------------------------------------------------------------------
// Applying the theme to egui::Visuals (egui 0.36.1)
// ---------------------------------------------------------------------
//
// Strategy: we don't rebuild Visuals/WidgetVisuals field by field (names
// change between versions: rounding -> corner_radius, expansion, etc.).
// Instead we start from Visuals::dark() (or ::light() for Latte) and
// override only colors and strokes via `..old`, letting the installed
// version supply every other field with its current names/defaults.

impl Theme {
    /// True for Latte, the only light theme among the four.
    fn is_light(&self) -> bool {
        // Latte has a light `base` (high luminance); the other three are dark.
        self.base.r() as u16 + self.base.g() as u16 + self.base.b() as u16 > 400
    }

    pub fn visuals(&self) -> egui::Visuals {
        let old = if self.is_light() {
            egui::Visuals::light()
        } else {
            egui::Visuals::dark()
        };
        ...
        egui::Visuals {
            ...
            widgets: egui::style::Widgets {
                noninteractive: Self::widget_visuals(old.widgets.noninteractive, self, self.base),
                inactive: Self::widget_visuals(old.widgets.inactive, self, self.surface0),
                hovered: Self::widget_visuals(old.widgets.hovered, self, self.surface2),
                active: Self::widget_visuals(old.widgets.active, self, self.surface1),
                open: Self::widget_visuals(old.widgets.open, self, self.surface0),
            },
            selection: egui::style::Selection {
                bg_fill: self.blue.gamma_multiply(0.4),
                stroke: Stroke {
                    color: self.overlay1,
...
/// Applies the theme to the egui context. Call this once at startup
/// (e.g. inside the `run_native` closure, on `cc.egui_ctx`).
pub fn set_theme(ctx: &egui::Context, theme: Theme) {
    ctx.set_visuals(theme.style().visuals);
}